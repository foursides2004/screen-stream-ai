...
A modular, end-to-end screen-reading AI assistant that captures a local display feed on Windows and makes streaming AI analysis accessible in real-time via a secure web browser UI. Runs entirely on a FREE tier using OpenRouter API routing. Monorepo: Next.js web app + lightweight Python client on Windows.

## Security & Environment

- **Never commit secrets**: All `.env*` files with real keys are gitignored
- **OpenRouter Free Tier**: Use free models (e.g., `google/gemini-2.0-flash-exp:free`)
